...
function getMessageStyle(minutes) {
    if (minutes < 180) return 'bg-red-50 text-red-800 border-l-4 border-red-500';
    if (minutes < 420) return 'bg-orange-50 text-orange-800 border-l-4 border-orange-500';
    if (minutes < 900) return 'bg-blue-50 text-blue-800 border-l-4 border-blue-500';
    return 'bg-purple-50 text-purple-800 border-l-4 border-purple-500';
}

function getStatusEmoji(minutes) {
    if (minutes < 180) return '🚨';
    if (minutes < 420) return '⚠️';
    if (minutes < 900) return '👍';
    return '🎯';
}

function getStatusTitle(minutes) {
    if (minutes < 180) return 'Critical Alert: Learning Deficit Detected';
    if (minutes < 420) return 'Positive Progress: Room for Growth';
    if (minutes < 900) return 'Excellent Commitment: On Track for Success';
